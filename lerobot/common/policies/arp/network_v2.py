...
class ARPNetwork(nn.Module):
        ...
        **kwargs,
        ...
        self.policy = arp.AutoRegressivePolicy(
            arp.ModelConfig(
                ...
                * n_arp_layers,
                tokens=[arp.TokenType.make(name="gripper", is_continuous=False, dim=1, dict_sizes=[2], embedding="discrete", predictor="class")]
                + visual_actions,
            )
        )
    ...
    def forward(self, batch):
        ...
        tk_names = ["o", "x", "y", "z", "gripper"] * horizon
        tk_ids = [name2id[name] for name in tk_names]
        chk_ids = []
        for h in range(horizon):
            chk_ids += [a + h * 2 for a in [0, 1, 1, 1, 1]]
        ...
        if self.training:
            ...
            for k in ["origin", "xaxis", "yaxis", "zaxis"]:
                ...
                heatmap = generate_heatmap_from_screen_pts(visual_action.flatten(0, 1), (vH, vW)).reshape(bs * horizon, vH, vW)
                contexts[f"smooth-heatmap@{k[0]}"] = heatmap
                actions[k[0]] = visual_action
            actions["gripper"] = batch["gripper_open"].unsqueeze(-1).repeat(1, 1, 2).float()

            tk_vals = torch.cat(
                [a[:, :, None, :] for a in [actions["o"], actions["x"], actions["y"], actions["z"], actions["gripper"]]], dim=2
            ).flatten(1, 2)
            ...
            pred_tks = self.policy.generate(
                prompt_tks, future_tk_chk_ids, contexts={"visual-tokens": visual_tokens, "visual-featmap": visual_featmap.flatten(0, 1)}, sample=False
            )
